	...
	while game_data['jeu_en_cours']:
		if not game_data['pause']:
			...
			if player.cycle % player.cycle_lenght == 0:
				...
				Sprite(type = choice(list(SpritesImages.keys())), cycle = player.cycle, position = (randint(0,1700), randint(0,800)))
			...
			level_text = Text(f'Level : {player.level()}', positions=(20,50), fixed=True)
			'''texte du niveau'''
